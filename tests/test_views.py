...
def test_get_cat_web_page_main(
        mock_get_stocks, mock_get_currency_rates, mock_get_top5_tran,
        mock_get_card_out, mock_greeting, mock_reading_operations,
...
@pytest.mark.parametrize("mock_operations_data, mock_profitable_cashback", [
    (
            [{"operation": "transaction details"}],
            [{"category": "Supermarkets", "cashback": 15}]
    ),
])
...
def test_get_cat_services_profitable_cashback(
        mock_get_profitable_cashback, mock_reading_operations,
...
@pytest.mark.parametrize("mock_operations_data, spending_data", [
    (
            [{"operation": "transaction details"}],
            {"category": "Supermarkets", "amount_spent": 150}
    ),
])
@patch('src.views.reading_operations_from_excel')
@patch('src.views.spending_by_category')
def test_get_cat_report_spending_by_category(
        mock_spending_by_category, mock_reading_operations,
