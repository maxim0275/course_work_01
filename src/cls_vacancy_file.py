...
class VacancyFile(BaseVacancyFile):
    ...
    def get_vacacny_fromfile(self, salary_mask: int = None, area_name_mask: str = None) -> list:
        ...
        if salary_mask and area_name_mask:
            return list(
                filter(
                    lambda x: area_name_mask.lower() in x["area_name"].lower() and x["salr_from"] > salary_mask,
                    loaded_data,
                )
            )
        elif salary_mask and area_name_mask is None:
            return list(filter(lambda x: x["salr_from"] > salary_mask, loaded_data))
        elif salary_mask is None and area_name_mask:
            return list(filter(lambda x: area_name_mask.lower() in x["area_name"].lower(), loaded_data))

        else:
            return list(filter(lambda x: x, loaded_data))
