...
def get_profitable_cashback(data, year_par="2021", month_par="12"):
    ...
    data_for_non_emt_cashback = data_for_period.loc[
        (data_for_period['Кэшбэк'].isna() != True) & (data_for_period['Статус'] == "OK")]

    # Удалить неиспользуемые данные
    data_for_period = []

    # Посчитать кешбэк по категориям
    series_cashback_cats = data_for_non_emt_cashback.groupby('Категория', dropna=False)['Кэшбэк'].sum().sort_values(
        ascending=False)
